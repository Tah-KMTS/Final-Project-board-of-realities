...
function Die({ value }) {
  return (
    <div className="grid h-20 w-20 grid-cols-3 grid-rows-3 gap-1 border-4 border-gray-800 bg-white p-2 shadow-lg">
      {Array.from({ length: 9 }).map((_, i) => {
        ...
        return (
          <div key={i} className="flex items-center justify-center">
            {hasPip && <div className="h-3 w-3 rounded-full bg-black" />}
          </div>
        )
      })}
    </div>
...
export default function DiceRollScreen() {
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-8 bg-[#0f1020] font-mono text-white">
      <h2 className="text-2xl font-bold text-yellow-300">Rolling for your starting Block...</h2>
      ...
      {!rolling && chosenBlock && (
        <div className="flex flex-col items-center gap-4 border-4 border-yellow-300 bg-[#1c1d3a] px-8 py-6 text-center">
          <p className="text-lg">
